...
func doJob(job *tJob) (terr network.TaskError) {
	_, err := os.Stat(job.Path)

	if err == nil {
		return network.NewTaskError(io.EOF, false)
	}
	...
	for _, s := range config.NetSourceMap[job.Type].Sources {
		...
		if ifZip && job.Unzip {
			...
			if err != nil {
				terr = network.NewTaskError(err, false)
				os.Remove(srcFile)
				os.Remove(desFile)
				continue
			} else {
...
func procTasks() error {
	...
	for idx, task := range cfg.Tasks {
		go func(idx int, task config.Task) {
			...
			job := tJob{Type: task.Type, Unzip: task.IfUnzip, Num: 0}
	...
	for i := 0; i < cfg.GoNum; i++ {
		go func() {
			for {
				...
				if err == nil {
					chanFinish <- fmt.Sprintf("[INFO] finished to download %s, source index %d, attempt num %d", job.Path, job.Index, job.Num)
				} else if err.IsEOF() {
					chanFinish <- fmt.Sprintf("[INFO] %s already exists", job.Path)
				} else {
					if job.Num <= maxTryNum || (err.IsTemporary() && job.Num <= maxReTryNum) {
						chanJob <- job
					} else {
						chanFinish <- fmt.Sprintf("[ERROR] failed to donwload %s, %s", job.Path, err)
					}
				}
			}
		}()
