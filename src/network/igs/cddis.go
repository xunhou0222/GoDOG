...
const (
	gnssCDDISUrl  = "https://cddis.nasa.gov/archive/"
...
func getCDDISProxyAuth(username, password string) network.TaskError {
	mutexAuthCDDIS.Lock()
	...
	// 1st request
	request, _ := http.NewRequest(http.MethodGet, gnssCDDISUrl, nil)
	request.Header.Add("User-Agent", network.HTTPUserAgent)
	response, err := client.Do(request)

	if err != nil {
		err = fmt.Errorf("in the 1st request, %s", err)
		return network.NewTaskError(err, true)
	...
	utf8Exp := regexp.MustCompile(`name="utf8" type="hidden" value="(.*?)"`)
	utf8Str := strings.ReplaceAll(utf8Exp.FindString(formStr), "\"", "")
	utf8Val := utf8Str[strings.LastIndexByte(utf8Str, '=')+1:]
	...
	if utf8Val == "" || authVal == "" || clientVal == "" || redirectVal == "" || responseVal == "" ||
	...
	request.Header.Set("User-Agent", network.HTTPUserAgent)
	...
	return network.NewTaskError(fmt.Errorf("ProxyAuth not found"), false)
}

func CDDISDownLoad(f *network.NetTask) network.TaskError {
	...
	fio, err := os.OpenFile(f.Path, flag, 0664)
	...
	for {
		...
		n, err = io.CopyN(fio, response.Body, 1024)
